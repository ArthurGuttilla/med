...
export async function POST(req: NextRequest) {
  try {
    ...
    // Get patient context
    const patient = db.prepare("SELECT * FROM patients WHERE id = ?").get(patient_id) as Record<string, unknown> | undefined;
    if (!patient) {
    ...
    if (!currentSessionId) {
      const sessionResult = db.prepare(
        "INSERT INTO chat_sessions (patient_id) VALUES (?)"
      ).run(patient_id);
    ...
    const systemPrompt = `You are a medical AI assistant helping a doctor review patient information and clinical history. You provide concise, clinically relevant responses to support medical decision-making.
...
${notes.map((n) => `### ${n.title} [${n.note_type}] - ${n.created_at}\n${n.content}`).join("\n\n") || "No notes documented"}
