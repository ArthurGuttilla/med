...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => {
        const updated = [...prev];
        updated[updated.length - 1] = { role: "assistant", content: "Sorry, an error occurred. Please try again.", streaming: false };
        return updated;
      });
    } finally {
      setSending(false);
            ...
            {sessions.length === 0 ? (
              ...
              sessions.map(s => (
                ...
                >
                  ...
                  <p className="text-gray-400 mt-0.5">{new Date(s.created_at).toLocaleDateString()} · {s.message_count} msgs</p>
                </button>
              ))
            )}
          </div>
      ...
      <div className="flex-1 flex flex-col bg-white rounded-xl border border-gray-200 overflow-hidden min-w-0">
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-gray-100">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-gray-400 hidden sm:block">claude-opus-4-6 · Patient context loaded</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-8">
              ...
              <p className="text-sm text-gray-400 mb-6 max-w-sm">The AI has full access to this patient&apos;s medical records, notes, vitals, and history.</p>
        ...
        <div className="border-t border-gray-100 p-4">
          ...
          <p className="text-xs text-gray-400 mt-2 px-1">Press Enter to send · Shift+Enter for new line · AI may make mistakes — always verify clinical information</p>
        </div>
      </div>
    </div>
  );
